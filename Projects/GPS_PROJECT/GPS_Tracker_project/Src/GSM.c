...
static void GSM_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GSM_GPIO, GSM_PIN_DTR, GPIO_PIN_RESET);
...
{      
      memset(buffer_GSM_Rx,'\0',sizeof(buffer_GSM_Rx));
      HAL_UART_Transmit_IT(&GSM_UART_HANDLE, "AT+CSCLK=1\r" , sizeof("AT+CSCLK=1\r"));
      HAL_Delay(3000);
}
...
void GSM_HttpRequest(void){
  ...
  while (buffer_GSM_Rx[i]!='\0' && r==0){
      if (buffer_GSM_Rx[i]=='2' && buffer_GSM_Rx[i+3]=='O' && buffer_GSM_Rx[i+4]=='K')
        r=1;
      else
        i++;
  }
...
void GSM_sleep(void){
  HAL_GPIO_WritePin(GSM_GPIO, GSM_PIN_DTR, GPIO_PIN_SET);
  HAL_Delay(4000);
  machine.GSM_module.GSM_Mode=GSM_SLEEP;
}
